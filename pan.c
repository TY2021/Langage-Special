...
short src_ln0 [] = {
	  0,  15,  15,  15,  16,  16,  16,  14, 
	 18,  13,  19,  20,  21,  22,  23,  12, 
	 25,  12,  25,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "Diner2.pml", 1, 18 },
...
{	int j, h = now._nr_pr;
	...
	case 0:	/* philosopher */
		...
		/* locals: */
		((P0 *)pptr(h))->n = 5;
		((P0 *)pptr(h))->left = (((((P0 *)this)->id-1)+((P0 *)this)->n)%((P0 *)this)->n);
		((P0 *)pptr(h))->right = ((P0 *)this)->id;
		((P0 *)pptr(h))->state = 4;
#ifdef VAR_RANGES
		logval("philosopher:state", ((P0 *)pptr(h))->state);
		logval("philosopher:id", ((P0 *)pptr(h))->id);
		logval("philosopher:n", ((P0 *)pptr(h))->n);
		logval("philosopher:left", ((P0 *)pptr(h))->left);
		logval("philosopher:right", ((P0 *)pptr(h))->right);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("BUSY"); return;
	case 2: Printf("FREE"); return;
	case 3: Printf("HUNGRY"); return;
	case 4: Printf("THINKING"); return;
	case 5: Printf("EATING"); return;
